...
const router=useRouter()
  ...
  return (
    <header
      className={`fixed top-0 w-full z-50 lg:h-28 h-20  transition-transform duration-300 border-b-[1px] border-gray-800 ${
    ...
    >
      <div className=" mx-auto lg:my-4 my-2 py-3 px-4 flex items-center justify-between text-white">
        ...
        <div className=" flex-row gap-8 hidden lg:flex ">
          <button onClick={()=>{router.push("/home")}} className="hover:text-red-600 text-[14px] font-exrabold transition duration-300">
            HOME
          </button>
          <nav className="  lg:flex space-x-9 font-semibold items-center">
            {navItems.map((items, index) => (
              ...
              >
                <a  href={items.link} className="hover:text-red-600 text-[14px] font-opensans font-exrabold transition duration-300">
                  {items.id}+
                </a>
      ...
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        ...
        <SheetContent side="left" className="bg-black opacity-90 border-none">
          ...
          <div className="space-x-9 p-4 font-semibold items-center">
            <button onClick={()=>{router.push("/home")}} className="hover:text-red-600 text-white text-[14px] font-opensans font-extrabold transition duration-300">
              HOME
            </button>
