 ...
 const [banners, setBanners] = useState([]);
  ...
  const onSubmit =async(e)=>{
   ...
   if(currentEditedId !=null){const result = await axios.put(
    `/api/services/banner/${currentEditedId}`, // ← Next.js App Router route
    formData,
    {
      headers: {
        "Content-Type": "application/json",
          ...
          `/api/services/banner`, // ← Next.js App Router route
          formData,
          {
            headers: {
              "Content-Type": "application/json",
...
<button className="bg-black text-white p-2 rounded"
      ...
      <div className="grid gap-2 md:grid-cols-4 lg-grid-cols-4 ">{banners && banners.length>0 ? banners.map((productItems) => (<ServiceBannerTile key={productItems._id} product={productItems} setCurrentEditedId={setCurrentEditedId} setFormData={setFormData} setCreateProductDialogue={setCreateProductDialogue} handleDelete={handleDelete}></ServiceBannerTile>)):<span>No banner addded</span>}
      </div>
