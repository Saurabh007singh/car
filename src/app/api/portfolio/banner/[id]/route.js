...
export async function PUT(req, { params }) {
  ...
  const banner = await Portfoliobanner.findById(id);
  if (!banner) {
...
export async function DELETE(req, { params }) {
  ...
  const deleted = await Portfoliobanner.findByIdAndDelete(id);
  if (!deleted) {
    return NextResponse.json({ success: false, message: "Banner not found" }, { status: 404 });
  }

  // 📁 Path to folder: /public/uploads/home/homebanner/{id}
  const folderPath = path.join(process.cwd(), "public", "uploads", "portfolio", "portfoliobanner", id);
...
export async function GET(req,{params}){
  ...
  const singleProduct=await Portfoliobanner.findById(id)
